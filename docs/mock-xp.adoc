...
NOTE: `jest.mock()` is only hoisted above the imports by SWC when `jest` is the global from `@types/jest`, not when it is imported from `@jest/globals` as here. The code under test is therefore loaded with a dynamic `import()` inside each test, after the mocks have been registered.
