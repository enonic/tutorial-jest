...
=== Mocking the Thymeleaf library

Notice that `/lib/thymeleaf`'s `render` mock simply returns the model object, so the actual html rendering is not triggered.

.src/jest/server/mockLibThymeleaf.ts
[source, typescript]
----
include::{sourcedir}src/jest/server/mockLibThymeleaf.ts[]
----
