...
Test files are transpiled by SWC (via `@swc/jest`), which strips the types without checking them — type errors in tests are caught by your editor. This is also why the config is a plain `.mjs` file: the project uses TypeScript 7, the native compiler, whose npm package no longer exports the classic compiler API (`createProgram`, `transpileModule`) that `ts-jest` and `ts-node` (needed for a `jest.config.ts`) depend on — only `tsc` and an explicitly unstable `typescript/unstable/*` surface remain.
